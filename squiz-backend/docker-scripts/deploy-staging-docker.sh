...
send_slack_notification()
{
    CURL_DATA='{
      "attachments": [
        ...
        {
          "color": "#36a64f",
          "title": "Go To GCloud/GCR",
          "title_link": "https://console.cloud.google.com/gcr/images/'${PROJECT_ID}'/GLOBAL/'${APPLICATION_ID}'?project='${PROJECT_ID}'&cloudshell=false&gcrImageListsize=30"
        },
        {
          "color": "#36a64f",
...
deploy_app(){
    gcloud app deploy ${APP_YAML} \
        --project ${PROJECT_ID} \
        --promote --no-stop-previous-version \
        --image-url gcr.io/${PROJECT_ID}/${APPLICATION_ID}:${BUILD_NO} \
        --version=${BUILD_NO}
    if [[ $? -eq 0 ]]; then
        send_slack_notification;
