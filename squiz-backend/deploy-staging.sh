...
APP_STAGING_YAML="$(pwd)/app-staging.yaml";

if [[ ! -f ${APP_PREPROD_YAML} || ! -f ${APP_STAGING_YAML} ]]; then
    echo "##### Deployment configuration file not found #####".
    exit;
fi

APPLICATION_ID="staging-search-and-filtering";
APPLICATION_NAME="Staging Search and Filtering";
